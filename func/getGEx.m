...
function [xLDimOut, geneExpression, geneAnnotes] =getGEx(cellTable, options)
    ...
    geneAnnotes=cellTable.Properties.VariableNames;
    iidl=[];
    for ii=1:length(geneAnnotes)
        if strcmpi(geneAnnotes{ii}(1:end-1), 'blank_')
            iidl=[iidl, ii];
        end
    end
    ...
    geneAnnotes=geneAnnotes(10:end-2);
    ...
    geneAnnotes(nanExpresionFlagColumn)=[];
    ...
    geneExpression = geneExpression./std(geneExpression);
