...
function [G,gStruct, cidOut, cellTypeVecU, haveZ, SIDIdxS, TOut]=creatAHCGraph(filename, options)

load('aneighs6.mat', 'aneighs');
...
for iAID=1:length(AIDIdx)
    ...
    for iBID=1:length(BIDU)  
        ...
        T2=T1(loopFlagBID, :);
         ...
         % nearNeighs = T2.neighs;
         % 
         %    for ian=1:size(nearNeighs)
         %        neighsi=nearNeighs(ian, :);
         %        neighsi=neighsi(neighsi>0);
         %        if ~isempty(neighsi)
         %        neighs_neighsi=nearNeighs(neighsi(:), :);
         %        falgs=any(neighs_neighsi==ian, 2);
         %        if ~all(falgs)
         %            check=1;
         %        end
         %        end
        ...
        elseif strcmpi(options.gMode,"knn")
            ...
            % if max(nearNeighs(:))>size(T2.neighs, 1)
            %     check=1;
            % end
            % nearNeighs=nearNeighs-min(T2.CellId)+1;
            % nearNeighsO = rangesearch(xyzcoords,xyzcoords, rEps);

            edges=cell(length(nearNeighs),1);
            for iin=1:length(nearNeighs)
                nearNeighsi=nearNeighs(iin, :);
                nearNeighsi=nearNeighsi(nearNeighsi>0);
                [~, nearNeighsi]=ismember(nearNeighsi, T2.CellId);
                % nearNeighsio=nearNeighsO{iin};
                % nearNeighsio=nearNeighsio(nearNeighsio~=iin);
                
                numNeighs=length(nearNeighsi);
                if numNeighs>0
                    edges1=repelem(iin, numNeighs, 1);
                    edges2=nearNeighsi;
                    edges2=edges2(:);
