...
function [G,gStruct,  h, BregmaAllU, AnimalIDIdx, TOut, edgeDistStd]=creatImGraph(T,folderName, options)
...
for iAnimalID=1:length(AnimalIDIdx)
    ...
    for iBregma=1:length(BregmaU)
        ...
        xshift=(BregmaU(iBregma)-1)*1e5/2;
        ...
        elseif strcmpi(options.gMode,"knn")
           ...
           neighsi=cell(size(zcoords, 1), 1);

           % parfor iz=1:size(zcoords, 1)
           %     dist=abs(zcoords-zcoords(iz, :));
           %     dist=max(dist,[], 2);
           %     neighsi{iz}=find(dist<=rEps);
           % end
           ...
           % neighsi= rangesearch(zcoords,zcoords,rEps,'Distance',@imgDistfun);
...
if plotGraph
    ...
    numHighlight=min(40, length(cellSubtypeVecU));
    % figure('WindowState' ,'maximize')
    figure
    ncoloro=jet(256);
    % ncolor=ncolor(end:-1:1, :);
    ncoloro=ncoloro(randperm(256), :);
    ...
    h=plot(G,'XData',xcoordsTotal,'YData',ycoordsTotal, 'EdgeColor',[0,0,0], 'MarkerSize',1);
        ...
        if iu<=numHighlight
            highlight(h,nodesAll(cellSubtypeVec==cellSubtypeVecU(iu)),'NodeColor',ncoloro(cellSubtypeVecU(iu), :), 'EdgeColor',[1,1,1])
        else
...
function D2 = imgDistfun(ZI,ZJ)
    ageDifference = abs(ZI(1)-ZJ(:,1));
    weightDifference = abs(ZI(2)-ZJ(:,2));
    D2 = max(ageDifference ,weightDifference);
end
