...
[G,gStruct, cellTypes]=creatGraph(folderName,cgOptions);
...
if isempty(options.alphabet)
    cTypeChars = ['ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz', char((198:198+length(cellTypes)-53))];
    cellTypesOne=cTypeChars(1:length(cellTypes));
else
...
[pathList, ~, nodeSections, pathWeights]=enumerateUSRKPathsTest(gStruct,enOptions);
...
[extMotif,textOut, commandText, background]=mtStreme(cPTypes=cPTypes0, cNTypes=cNTypes0,cPHTypes=cPTypes,cNHTypes=cNTypes(1:length(cPTypes)),pSeq=posSeq, ...
    ...
    gTrainNum=options.gTrainNum, isEnrich=options.isEnrich, scIterMax=options.scIterMax, patience=options.patience,NREF=options.NREF, NEVAL=options.NEVAL);
...
for iexMotif=1:numExtMotifs
    ...
    [~, hfig, cTypeChars] = seqlogoRet(exMotifi);
